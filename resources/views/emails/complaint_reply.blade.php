...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background-color: #f8f9fa; padding: 20px; border-radius: 10px; border: 1px solid #ddd;">
        ...
        <p>Kính gửi quý khách <strong>{{ $complaint->customer->user->name ?? 'Khách hàng' }}</strong>,</p>
        
        <p>Cảm ơn quý khách đã gửi phản hồi và khiếu nại về đơn hàng <strong>#{{ $complaint->order->code ?? '' }}</strong> cho CozyHNA. Quản lý cửa hàng đã tiếp nhận và xem xét sự việc.</p>
        ...
        <ul style="background-color: #fff; padding: 15px 15px 15px 35px; border-radius: 8px; margin-top: 0;">
            ...
            <li><strong>Đối tượng liên quan:</strong> {{ $complaint->target_person ?? 'Không có' }}</li>
