                    ...
                    <div class="grid grid-cols-2 gap-lg">
                        <div class="col-span-2">
                            ...
                            <select name="target" class="w-full border-outline-variant rounded-lg p-md focus:border-primary focus:ring-1 focus:ring-primary text-body-md">
                                <option value="product" {{ old('target') == 'product' ? 'selected' : '' }}>Giảm giá sản phẩm</option>
                                <option value="shipping" {{ old('target') == 'shipping' ? 'selected' : '' }}>Giảm phí vận chuyển (Freeship)</option>
                            </select>
                        </div>
                        <div class="col-span-2">
                            ...
                            <div class="flex flex-wrap gap-4 mt-2">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="all" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('all', old('target_audience', ['all'])) ? 'checked' : '' }}>
                                    <span class="text-body-md">Tất cả khách hàng</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="Member" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('Member', old('target_audience', [])) ? 'checked' : '' }}>
                                    <span class="text-body-md">Thành viên</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="Silver" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('Silver', old('target_audience', [])) ? 'checked' : '' }}>
                                    <span class="text-body-md">Hạng Bạc</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="Gold" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('Gold', old('target_audience', [])) ? 'checked' : '' }}>
                                    <span class="text-body-md">Hạng Vàng</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="Diamond" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('Diamond', old('target_audience', [])) ? 'checked' : '' }}>
            ...
            <div class="col-span-12 lg:col-span-4 space-y-lg">
                ...
                <section class="bg-surface-container-lowest rounded-xl p-xl border border-outline-variant shadow-sm">
                    ...
                    <div class="space-y-lg">
                        ...
                        <label class="flex items-center gap-md p-md border border-outline-variant rounded-lg cursor-pointer hover:bg-surface-container-low transition-colors">
                            <div class="relative flex items-center">
                                <input type="checkbox" name="is_one_time_use" class="peer sr-only" value="1" {{ old('is_one_time_use') ? 'checked' : '' }}>
                                <div class="w-10 h-6 bg-surface-variant rounded-full peer peer-checked:bg-primary transition-colors"></div>
                                <div class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-4 shadow-sm"></div>
                            </div>
