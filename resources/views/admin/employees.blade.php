...
<p class="text-on-surface-variant font-label-md">{{ __('On Duty Now') }}</p>
...
<p class="text-on-surface-variant font-label-md">{{ __('Avg Performance') }}</p>
...
<p class="text-on-surface-variant font-label-md">{{ __('Upcoming Today') }}</p>
...
<div>
<h4 class="font-title-lg text-title-lg text-on-surface">{{ __('Staff Directory') }}</h4>
<p class="font-label-md text-on-surface-variant">{{ __('Manage and track your beverage service team') }}</p>
</div>
...
<span class="material-symbols-outlined text-[18px]">filter_list</span>{{ __('Filter') }}</button>
</div>
<div class="overflow-x-auto">
<table class="w-full text-left border-collapse">
<thead>
<tr class="bg-surface-container-low/50">
<th class="px-6 py-4 font-label-sm text-on-surface-variant uppercase tracking-wider">{{ __('Employee') }}</th>
<th class="px-6 py-4 font-label-sm text-on-surface-variant uppercase tracking-wider">{{ __('Role') }}</th>
<th class="px-6 py-4 font-label-sm text-on-surface-variant uppercase tracking-wider">Trạng thái</th>
<th class="px-6 py-4 font-label-sm text-on-surface-variant uppercase tracking-wider">{{ __('Performance') }}</th>
<th class="px-6 py-4 font-label-sm text-on-surface-variant uppercase tracking-wider">{{ __('Shift') }}</th>
...
<div>
<p class="font-body-md text-on-surface font-semibold">{{ __('Sarah Jenkins') }}</p>
<p class="text-label-md text-on-surface-variant">sarah.j@cozyhna.com</p>
</div>
</div>
</td>
<td class="px-6 py-4 text-body-md text-on-surface">{{ __('Lead Barista') }}</td>
<td class="px-6 py-4">
<span class="px-3 py-1 rounded-full bg-primary/10 text-primary text-[11px] font-bold uppercase tracking-wider">{{ __('On Shift') }}</span>
</td>
<td class="px-6 py-4">
...
<div>
<p class="font-body-md text-on-surface font-semibold">{{ __('Mark Thompson') }}</p>
<p class="text-label-md text-on-surface-variant">mark.t@cozyhna.com</p>
</div>
</div>
</td>
<td class="px-6 py-4 text-body-md text-on-surface">{{ __('Kitchen Manager') }}</td>
<td class="px-6 py-4">
<span class="px-3 py-1 rounded-full bg-outline-variant/30 text-on-surface-variant text-[11px] font-bold uppercase tracking-wider">{{ __('Off Duty') }}</span>
</td>
<td class="px-6 py-4">
<div class="flex items-center gap-1">
<span class="material-symbols-outlined text-[18px] text-tertiary" style="font-variation-settings: 'FILL' 1;">star</span>
...
<div>
<p class="font-body-md text-on-surface font-semibold">{{ __('David Chen') }}</p>
<p class="text-label-md text-on-surface-variant">david.c@cozyhna.com</p>
</div>
</div>
</td>
<td class="px-6 py-4 text-body-md text-on-surface">{{ __('Delivery Associate') }}</td>
<td class="px-6 py-4">
<span class="px-3 py-1 rounded-full bg-tertiary-container/20 text-tertiary text-[11px] font-bold uppercase tracking-wider">{{ __('On Leave') }}</span>
</td>
<td class="px-6 py-4">
<div class="flex items-center gap-1">
<span class="material-symbols-outlined text-[18px] text-tertiary" style="font-variation-settings: 'FILL' 1;">star</span>
...
<td class="px-6 py-4 text-body-md text-on-surface-variant">{{ __('N/A') }}</td>
<td class="px-6 py-4 text-right">
<button class="p-2 rounded-full hover:bg-surface-container-high text-on-surface-variant opacity-0 group-hover:opacity-100 transition-all">
<span class="material-symbols-outlined">more_vert</span>
</button>
...
<div>
<p class="font-body-md text-on-surface font-semibold">{{ __('Elena Rodriguez') }}</p>
<p class="text-label-md text-on-surface-variant">elena.r@cozyhna.com</p>
</div>
</div>
</td>
<td class="px-6 py-4 text-body-md text-on-surface">{{ __('Junior Barista') }}</td>
<td class="px-6 py-4">
<span class="px-3 py-1 rounded-full bg-primary/10 text-primary text-[11px] font-bold uppercase tracking-wider">{{ __('On Shift') }}</span>
</td>
<td class="px-6 py-4">
...
<p class="text-label-md text-on-surface-variant">{{ __('Showing 1-10 of 24 staff members') }}</p>
<div class="flex gap-2">
<button class="px-3 py-1 border border-outline-variant rounded-md hover:bg-surface-container-high transition-colors text-on-surface-variant">{{ __('Previous') }}</button>
...
<button class="px-3 py-1 border border-outline-variant rounded-md hover:bg-surface-container-high transition-colors text-on-surface-variant">{{ __('Next') }}</button>
</div>
</div>
</div>
<!-- Management Sidebar -->
...
<span class="font-label-md">{{ __('Schedule Shift') }}</span>
<span class="material-symbols-outlined">schedule</span>
</button>
<button class="w-full flex items-center justify-between p-4 bg-surface-container-low text-on-surface rounded-xl border border-outline-variant hover:bg-surface-container-high transition-all active:scale-[0.98]">
<span class="font-label-md">{{ __('Export Payroll') }}</span>
...
<h4 class="font-title-lg text-title-lg text-on-surface mb-6">{{ __('Coming Up Next') }}</h4>
...
<p class="font-body-md text-on-surface font-semibold">{{ __('James Wilson') }}</p>
...
<p class="font-body-md text-on-surface font-semibold">{{ __('Sophia L.') }}</p>
<p class="text-label-md text-on-surface-variant">{{ __('Kitchen Prep') }}</p>
</div>
</div>
<!-- Arrival 3 -->
<div class="flex gap-4">
...
<p class="font-body-md text-on-surface font-semibold">{{ __('Ryan Park') }}</p>
<p class="text-label-md text-on-surface-variant">{{ __('Delivery Night Team') }}</p>
</div>
</div>
</div>
<button class="w-full mt-6 py-2 text-primary font-label-md hover:underline">{{ __('View Full Schedule') }}</button>
</div>
...
<h4 class="font-title-lg text-title-lg text-primary mb-2">{{ __('Team Insight') }}</h4>
<p class="text-body-md text-on-primary-container leading-relaxed">{{ __('Team productivity is up') }}<span class="font-bold">14%</span>{{ __('this week. Sarah Jenkins has maintained a perfect 5.0 rating for 12 consecutive shifts.') }}</p>
