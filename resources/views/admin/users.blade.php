...
<main class="ml-0 md:ml-[280px] pt-16 min-h-screen p-8 bg-surface-container-lowest">
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
            <!-- Tabs Header -->
            <div class="flex bg-surface-container-low p-1 rounded-lg w-max">
            ...
            <!-- Search Form -->
            <form action="/admin/users" method="GET" class="flex items-center w-full md:w-auto">
                <div class="relative w-full md:w-80">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant text-[20px]">search</span>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm tên, email, sđt..." class="w-full pl-10 pr-4 py-2 rounded-lg border border-outline-variant bg-surface focus:border-primary focus:ring-primary text-sm">
                </div>
                <button type="submit" class="ml-2 px-4 py-2 bg-surface-container-high text-on-surface rounded-lg text-sm hover:bg-surface-container-highest transition-colors font-medium">Tìm</button>
            </form>
        </div>

        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const activeTab = localStorage.getItem('activeUsersTab') || 'employees';
                switchTab(activeTab);
            });
            ...
            function switchTab(tabId) {
                ...
                localStorage.setItem('activeUsersTab', tabId);
            }
        </script>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-surface-container-low border-b border-outline-variant/30">
                            ...
                            @if(check_permission('assign_roles') || session('role_code') === 'admin')
                            <th class="p-4 font-label-sm text-on-surface-variant uppercase tracking-wider">Thao tác</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-outline-variant/30">
                        ...
                        <tr class="hover:bg-surface-container-low/50 transition-colors">
                            ...
                            <td class="p-4">
                                @if($user->status)
                                    <span class="px-2 py-1 rounded-md text-xs font-semibold bg-primary-container text-primary">Hoạt động</span>
                                @else
                                    <span class="px-2 py-1 rounded-md text-xs font-semibold bg-error-container text-error">Đã khóa</span>
                                @endif
                            </td>
                            <td class="p-4">
                                @if($user->role)
                            ...
                            <td class="p-4 flex items-center gap-2">
                                ...
                                @endif
                                @if(session('role_code') === 'admin' && (!$user->role || $user->role->code !== 'admin'))
                                <form action="/admin/users/{{ $user->id }}/toggle-status" method="POST" class="m-0">
                                    @csrf
                                    <button type="submit" class="px-3 py-1.5 text-sm rounded-lg font-semibold transition-colors {{ $user->status ? 'bg-error-container text-error hover:bg-error/20' : 'bg-primary-container text-primary hover:bg-primary/20' }}">
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="p-8 text-center text-on-surface-variant">Chưa có người dùng nào.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
