            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="p-4 font-semibold text-gray-600 text-sm">Hình ảnh</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="p-4">
                            @if($review->media && is_array($review->media))
                                <div class="flex gap-2">
                                    @foreach($review->media as $mediaUrl)
                                        @if(preg_match('/\.(mp4|mov|avi)$/i', $mediaUrl))
                                            <a href="{{ $mediaUrl }}" target="_blank" class="block w-12 h-12 bg-gray-100 rounded flex items-center justify-center border border-gray-200" title="Xem Video">
                                                <span class="material-symbols-outlined text-gray-400">play_circle</span>
                                            </a>
                                        @else
                                            <a href="{{ $mediaUrl }}" target="_blank">
                                                <img src="{{ $mediaUrl }}" class="w-12 h-12 object-cover rounded shadow-sm border border-gray-200">
                                            </a>
                                        @endif
                                    @endforeach
                                </div>
