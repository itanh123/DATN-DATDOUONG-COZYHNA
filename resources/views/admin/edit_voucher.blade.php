        ...
        <div class="grid grid-cols-12 gap-lg">
            ...
            <div class="col-span-12 lg:col-span-8 space-y-lg">
                ...
                <section class="bg-surface-container-lowest rounded-xl p-xl border border-outline-variant shadow-sm">
                    ...
                    <div class="grid grid-cols-2 gap-lg">
                        <div class="col-span-2">
                            <label class="block font-label-md text-label-md text-on-surface-variant mb-xs">Loại mã giảm giá <span class="text-error">*</span></label>
                            <select name="target" class="w-full border-outline-variant rounded-lg p-md focus:border-primary focus:ring-1 focus:ring-primary text-body-md">
                                <option value="product" {{ old('target', $voucher->target) == 'product' ? 'selected' : '' }}>Giảm giá sản phẩm</option>
                                <option value="shipping" {{ old('target', $voucher->target) == 'shipping' ? 'selected' : '' }}>Giảm phí vận chuyển (Freeship)</option>
                            </select>
                        </div>
                        <div class="col-span-2">
                            <label class="block font-label-md text-label-md text-on-surface-variant mb-xs">Áp dụng cho Hạng <span class="text-error">*</span></label>
                            <div class="flex flex-wrap gap-4 mt-2">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="all" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('all', old('target_audience', is_array($voucher->target_audience) ? $voucher->target_audience : (json_decode($voucher->target_audience, true) ?? ['all']))) ? 'checked' : '' }}>
                                    <span class="text-body-md">Tất cả khách hàng</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="Member" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('Member', old('target_audience', is_array($voucher->target_audience) ? $voucher->target_audience : (json_decode($voucher->target_audience, true) ?? []))) ? 'checked' : '' }}>
                                    <span class="text-body-md">Thành viên</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="Silver" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('Silver', old('target_audience', is_array($voucher->target_audience) ? $voucher->target_audience : (json_decode($voucher->target_audience, true) ?? []))) ? 'checked' : '' }}>
                                    <span class="text-body-md">Hạng Bạc</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="Gold" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('Gold', old('target_audience', is_array($voucher->target_audience) ? $voucher->target_audience : (json_decode($voucher->target_audience, true) ?? []))) ? 'checked' : '' }}>
                                    <span class="text-body-md">Hạng Vàng</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="target_audience[]" value="Diamond" class="rounded text-primary focus:ring-primary w-5 h-5 border-outline-variant" {{ in_array('Diamond', old('target_audience', is_array($voucher->target_audience) ? $voucher->target_audience : (json_decode($voucher->target_audience, true) ?? []))) ? 'checked' : '' }}>
                                    <span class="text-body-md">Hạng Kim Cương</span>
                                </label>
                            </div>
                        </div>
                        <div>
