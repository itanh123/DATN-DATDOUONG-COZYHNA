                    ...
                    <div class="grid grid-cols-2 gap-lg">
                        <div class="col-span-2">
                            ...
                            <select name="discount_target" id="discount_target" class="w-full border-outline-variant rounded-lg p-md focus:border-primary focus:ring-1 focus:ring-primary text-body-md" onchange="toggleProductSelection()">
                                <option value="order" {{ old('discount_target', $voucher->discount_target) == 'order' ? 'selected' : '' }}>Giảm giá toàn đơn hàng</option>
                                <option value="shipping" {{ old('discount_target', $voucher->discount_target) == 'shipping' ? 'selected' : '' }}>Miễn phí / Giảm phí vận chuyển</option>
                                <option value="product" {{ old('discount_target', $voucher->discount_target) == 'product' ? 'selected' : '' }}>Giảm giá cho sản phẩm cụ thể</option>
                            </select>
                        </div>
                        <div class="col-span-2" id="product_selection_container" style="{{ old('discount_target', $voucher->discount_target) == 'product' ? '' : 'display: none;' }}">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    ...
                                    @php
                                        ...
                                        if (is_array(old('product_ids'))) {
                                            $isChecked = in_array($product->id, old('product_ids'));
                                        } else {
                    ...
                    <div class="space-y-lg">
                        ...
                        <label class="flex items-center gap-xs mt-md cursor-pointer group">
                            <div class="relative flex items-center justify-center w-6 h-6">
                                <input name="is_hot" type="checkbox" value="1" {{ old('is_hot', $voucher->is_hot) ? 'checked' : '' }} class="peer appearance-none w-5 h-5 border-2 border-outline-variant rounded bg-surface-container-lowest checked:bg-error checked:border-error transition-all"/>
