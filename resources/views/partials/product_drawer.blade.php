            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.status === 'added') {
                        ...
                        if (window.favoriteProductIds && !window.favoriteProductIds.includes(currentProductId)) {
                            window.favoriteProductIds.push(currentProductId);
                        }
                    } else {
                        icon.style.fontVariationSettings = "'FILL' 0";
                        btn.classList.remove('bg-error-container');
                        btn.classList.add('bg-surface/90');
                        if (window.favoriteProductIds) {
                            window.favoriteProductIds = window.favoriteProductIds.filter(id => id !== currentProductId);
                        }
                    }
                    
                    // Also sync the icons in the product cards
                    document.querySelectorAll('.favorite-icon-' + currentProductId).forEach(cardIcon => {
                        if (data.status === 'added') {
                            cardIcon.style.fontVariationSettings = "'FILL' 1";
                            cardIcon.classList.add('text-error');
                        } else {
                            cardIcon.style.fontVariationSettings = "'FILL' 0";
                            cardIcon.classList.remove('text-error');
                        }
                    });
            ...
            if (element && element.dataset.product) {
                try {
                    ...
                    if (window.favoriteProductIds && window.favoriteProductIds.includes(currentProductId)) {
                        ...
                        favBtn.classList.add('bg-error-container');
                        favBtn.classList.remove('bg-surface/90');
                        favIcon.classList.add('text-error');
                    } else {
                        favIcon.style.fontVariationSettings = "'FILL' 0";
                        favBtn.classList.remove('bg-error-container');
                        favBtn.classList.add('bg-surface/90');
