                            ...
                            if (approvedReviews.length > 0) {
                                approvedReviews.forEach(review => {
                                    ...
                                    if (review.media && Array.isArray(review.media)) {
                                        reviewHtml += `<div class="flex gap-2 mt-2 overflow-x-auto hide-scrollbar">`;
                                        review.media.forEach(mediaUrl => {
                                            if (mediaUrl.match(/\.(mp4|mov|avi)$/i)) {
                                                reviewHtml += `<video src="${mediaUrl}" controls class="w-full max-w-[200px] rounded-lg object-cover border border-outline-variant/20"></video>`;
                                            } else {
                                                reviewHtml += `<img src="${mediaUrl}" class="w-full max-w-[200px] rounded-lg object-cover border border-outline-variant/20" alt="Review Media">`;
                                            }
                                        });
                                        reviewHtml += `</div>`;
                                    }
