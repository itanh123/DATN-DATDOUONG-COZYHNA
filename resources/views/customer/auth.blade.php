...
<main class="min-h-screen flex items-center justify-center relative bg-cover bg-center" style="background-image: url('https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&q=80');">
    ...
    <div class="relative z-10 w-full max-w-[420px] mx-4 p-8 rounded-3xl backdrop-blur-md bg-white/20 border border-white/30 shadow-2xl">
        <div class="form-transition" id="authContent">
            <h2 class="text-3xl font-bold text-white mb-2" id="authHeading">{{ __('Login') }}</h2>
            <p class="text-white/90 text-sm mb-8" id="authSubtext">{{ __('Welcome back please login to your account') }}</p>
            ...
            <form id="authForm" action="/login" method="POST" class="space-y-4">
                ...
                <div class="hidden transform transition-all duration-300 space-y-4" id="signupFields">
                    <div>
                        <div class="relative">
                            <input name="username" class="w-full bg-transparent border border-white/50 rounded-xl px-4 py-3 text-white placeholder-white/80 focus:outline-none focus:border-white focus:ring-1 focus:ring-white transition-all" placeholder="{{ __('Full Name') }}" type="text"/>
                            <span class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-white/80">person</span>
                        </div>
                    </div>
                    <div>
                        <div class="relative">
                            <input name="phone" class="w-full bg-transparent border border-white/50 rounded-xl px-4 py-3 text-white placeholder-white/80 focus:outline-none focus:border-white focus:ring-1 focus:ring-white transition-all" placeholder="{{ __('Phone Number') }}" type="tel"/>
                ...
                <div>
                    <div class="relative">
                        <input name="email" class="w-full bg-transparent border border-white/50 rounded-xl px-4 py-3 text-white placeholder-white/80 focus:outline-none focus:border-white focus:ring-1 focus:ring-white transition-all" placeholder="{{ __('Email Address') }}" type="email" required/>
                ...
                <div class="flex justify-between items-center mt-2">
                    <div class="flex items-center gap-2" id="termsCheck">
                        ...
                        <label class="text-sm text-white/90" id="termsLabel">{{ __('Remember me') }}</label>
                    </div>
                    <a class="text-sm text-white/90 hover:underline transition-opacity" href="/forgot-password" id="forgotPass">{{ __('Forgot password?') }}</a>
                </div>
            ...
            <div class="mt-6 text-center text-sm text-white/90" id="toggleHint">{{ __('Don\'t have an account?') }}<button type="button" onclick="toggleAuth('signup')" class="font-bold text-white hover:underline">{{ __('Signup') }}</button>
            </div>
            
            <div class="mt-6 text-center text-xs text-white/70">{{ __('Created by') }}<span class="font-bold text-white/90">{{ __('CozyHNA') }}</span>
            </div>
        </div>
    </div>
</main>
...
<script>
        ...
        function toggleAuth(mode) {
            ...
            if (mode === 'signup') {
                ...
                toggleHint.innerHTML = 'Already have an account? <button type="button" onclick="toggleAuth(\'login')" class="font-bold text-white hover:underline">{{ __('Login') }}</button>';
                ...
                toggleHint.innerHTML = "Don't have an account? <button type=\"button\" onclick=\"toggleAuth('signup')\" class=\"font-bold text-white hover:underline\">{{ __('Signup') }}</button>";
