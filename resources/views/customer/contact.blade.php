...
<h1 class="font-display-lg text-display-lg mb-md text-primary">{{ __('Get in Touch') }}</h1>
...
<p class="text-label-md text-label-md text-primary mt-xs uppercase">{{ __('Mon - Fri, 9am - 6pm EST') }}</p>
</div>
<div class="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant/30 shadow-sm hover:shadow-md transition-shadow">
<div class="flex items-center gap-md mb-sm">
<div class="w-12 h-12 rounded-full bg-secondary-container flex items-center justify-center">
...
<h3 class="font-title-lg text-title-lg">{{ __('Email') }}</h3>
</div>
<p class="text-on-surface-variant">hello@cozyhna.com</p>
<p class="text-label-md text-label-md text-primary mt-xs uppercase">{{ __('Response within 24 hours') }}</p>
</div>
<div class="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant/30 shadow-sm hover:shadow-md transition-shadow">
<div class="flex items-center gap-md mb-sm">
<div class="w-12 h-12 rounded-full bg-secondary-container flex items-center justify-center">
...
<h3 class="font-title-lg text-title-lg">{{ __('Headquarters') }}</h3>
</div>
<p class="text-on-surface-variant">742 Vitality Lane, Green District</p>
<p class="text-on-surface-variant">{{ __('Portland, OR 97201') }}</p>
</div>
<!-- Quick FAQ Link -->
<div class="bg-primary-container/10 p-lg rounded-xl border border-primary/20 mt-lg">
<h4 class="font-title-lg text-title-lg text-primary mb-xs">{{ __('Need a quick answer?') }}</h4>
<p class="text-body-md text-body-md mb-md">{{ __('Check our Frequently Asked Questions for instant help.') }}</p>
<a class="inline-flex items-center gap-xs font-semibold text-primary hover:underline" href="#">{{ __('Visit FAQ Center') }}<span class="material-symbols-outlined text-sm" data-icon="arrow_forward">arrow_forward</span>
</a>
</div>
</div>
...
<h2 class="font-headline-md text-headline-md mb-xl">{{ __('Send us a Message') }}</h2>
<form class="grid grid-cols-1 md:grid-cols-2 gap-lg" id="contactForm">
<div class="space-y-xs">
<label class="font-label-md text-label-md text-on-surface-variant uppercase ml-base" for="name">{{ __('Full Name') }}</label>
<input class="w-full px-md py-sm rounded-lg border border-outline-variant focus:ring-2 focus:ring-primary focus:border-primary bg-white transition-all outline-none" id="name" name="name" placeholder="{{ __('John Doe') }}" required="" type="text"/>
</div>
<div class="space-y-xs">
<label class="font-label-md text-label-md text-on-surface-variant uppercase ml-base" for="email">{{ __('Email Address') }}</label>
...
<label class="font-label-md text-label-md text-on-surface-variant uppercase ml-base" for="subject">{{ __('Subject') }}</label>
...
<option value="product">{{ __('Product Inquiry') }}</option>
<option value="partnership">{{ __('Partnership') }}</option>
<option value="other">{{ __('Other') }}</option>
</select>
</div>
<div class="md:col-span-2 space-y-xs">
<label class="font-label-md text-label-md text-on-surface-variant uppercase ml-base" for="message">{{ __('Message') }}</label>
<textarea class="w-full px-md py-sm rounded-lg border border-outline-variant focus:ring-2 focus:ring-primary focus:border-primary bg-white transition-all outline-none resize-none" id="message" name="message" placeholder="{{ __('How can we help you today?') }}" required="" rows="5"></textarea>
</div>
<div class="md:col-span-2 flex justify-end pt-md">
<button class="bg-primary-container text-on-primary-container font-headline-md text-headline-md px-xl py-md rounded-full shadow-md hover:shadow-lg active:scale-95 transition-all flex items-center gap-md" type="submit">{{ __('Send Message') }}<span class="material-symbols-outlined" data-icon="send">send</span>
</button>
</div>
</form>
</div>
...
<div>
<h2 class="font-headline-lg text-headline-lg text-on-surface">{{ __('Store Locations') }}</h2>
<p class="text-on-surface-variant">{{ __('Find a CozyHNA experience near you.') }}</p>
</div>
...
<h4 class="font-title-lg text-title-lg text-primary">{{ __('Portland Flagship') }}</h4>
<p class="text-sm mb-md">742 Vitality Lane, OR</p>
<div class="flex items-center gap-xs text-secondary font-semibold">
<span class="w-2 h-2 rounded-full bg-secondary animate-pulse"></span>{{ __('Open until 8:00 PM') }}</div>
</div>
</div>
...
<div>
<span class="text-label-sm text-label-sm bg-secondary-container text-on-secondary-container px-sm py-1 rounded-full uppercase tracking-wider">{{ __('Seattle Central') }}</span>
<h4 class="font-title-lg text-title-lg mt-sm">{{ __('Pike Place Market') }}</h4>
<p class="text-body-md text-body-md text-on-surface-variant mt-xs">{{ __('A bustling hub for our fresh organic cold brews.') }}</p>
</div>
...
<div class="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant/30 flex-1 flex flex-col justify-between group cursor-pointer hover:border-primary/50 transition-all">
<div>
<span class="text-label-sm text-label-sm bg-secondary-container text-on-secondary-container px-sm py-1 rounded-full uppercase tracking-wider">{{ __('San Francisco') }}</span>
<h4 class="font-title-lg text-title-lg mt-sm">{{ __('Mission District') }}</h4>
<p class="text-body-md text-body-md text-on-surface-variant mt-xs">{{ __('Experience our limited edition botanical infusions.') }}</p>
</div>
<div class="flex justify-between items-center mt-md">
<span class="text-label-md text-label-md text-on-surface-variant">{{ __('Open Now') }}</span>
<span class="material-symbols-outlined text-primary group-hover:translate-x-1 transition-transform" data-icon="directions">directions</span>
</div>
</div>
</div>
