...
<div id="reviewModal" class="fixed inset-0 z-50 hidden bg-black/50 flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 relative">
        ...
        <form action="/customer/reviews" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-4">
                <label class="block text-label-md font-bold mb-2">Hình ảnh / Video đính kèm (Tùy chọn)</label>
                <input type="file" name="media[]" id="mediaInput" accept="image/*,video/mp4,video/quicktime,video/x-msvideo" multiple class="w-full p-2 border border-outline-variant rounded-lg focus:border-primary focus:ring-0 text-body-md" />
                <p class="text-label-sm text-on-surface-variant mt-1">Tối đa 2 ảnh hoặc 1 video (tối đa 10s)</p>
            </div>
...
<script>
        ...
        document.getElementById('mediaInput')?.addEventListener('change', function(e) {
                ...
                if (file.type.startsWith('video/')) {
                    ...
                    video.onloadedmetadata = function() {
                        ...
                        if (video.duration > 10) {
                            alert('Video không được vượt quá 10 giây.');
                            e.target.value = '';
                        }
                    }
            ...
            if (hasVideo && (files.length > 1)) {
                alert('Bạn chỉ có thể tải lên 1 video (nếu đã chọn video thì không được tải thêm ảnh/video khác).');
                e.target.value = '';
            }
        });
</script>
