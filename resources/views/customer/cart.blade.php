                ...
                <div class="lg:w-1/3 w-full sticky top-24">
                    <div class="premium-card rounded-2xl p-6">
                        ...
                        <div id="discountContainer" class="flex justify-between items-center mb-3 {{ (!isset($discountAmount) || $discountAmount <= 0) ? 'hidden' : '' }}">
                            <span class="text-[14px] text-error">Giảm giá sản phẩm</span>
                            <span class="font-semibold text-error" id="discountDisplay">-{{ number_format($discountAmount ?? 0, 0, ',', '.') }} đ</span>
                        </div>

                        <div id="shippingDiscountContainer" class="flex justify-between items-center mb-3 {{ (!isset($shippingDiscountAmount) || $shippingDiscountAmount <= 0) ? 'hidden' : '' }}">
                            <span class="text-[14px] text-error">Giảm phí vận chuyển</span>
                            <span class="font-semibold text-error" id="shippingDiscountDisplay">-{{ number_format($shippingDiscountAmount ?? 0, 0, ',', '.') }} đ</span>
                        </div>

                        <div class="mb-5 space-y-3">
                            <!-- Voucher Sản Phẩm -->
                            <div class="border border-outline-variant/30 rounded-lg p-3">
                                <p class="text-[12px] text-on-surface-variant mb-2 uppercase tracking-wider font-bold">Mã giảm giá sản phẩm</p>
                                @if(session('applied_voucher'))
                                    <div class="bg-success-container text-on-success-container p-2 rounded-lg flex items-center justify-between">
                                        <div>
                                            <p class="font-bold text-[13px]">{{ session('applied_voucher')['code'] }}</p>
                                            <p class="text-[11px]">Giảm: -{{ number_format(session('applied_voucher')['discount_amount'], 0, ',', '.') }} đ</p>
                                        </div>
                                        <form action="{{ route('vouchers.remove') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="target" value="product">
                                            <button type="submit" class="text-error hover:bg-white/20 p-1 rounded-full transition-colors flex" title="Bỏ mã">
                                                <span class="material-symbols-outlined text-[16px]">close</span>
                                            </button>
                                        </form>
                                    </div>
                                @else
                                    <button type="button" onclick="openVoucherModal('product')" class="w-full py-2 bg-primary/10 text-primary text-[13px] font-bold rounded-lg hover:bg-primary/20 transition-colors flex items-center justify-center gap-1.5">
                                        <span class="material-symbols-outlined text-[16px]">add_circle</span> Chọn hoặc nhập mã
                                    </button>
                                @endif
                            </div>
                            ...
                            <!-- Voucher Phí Ship -->
                            <div class="border border-outline-variant/30 rounded-lg p-3">
                                <p class="text-[12px] text-on-surface-variant mb-2 uppercase tracking-wider font-bold">Mã miễn phí vận chuyển</p>
                                @if(session('applied_shipping_voucher'))
                                    <div class="bg-success-container text-on-success-container p-2 rounded-lg flex items-center justify-between">
                                        <div>
                                            <p class="font-bold text-[13px]">{{ session('applied_shipping_voucher')['code'] }}</p>
                                            <p class="text-[11px]">Giảm phí ship: -{{ number_format($shippingDiscountAmount ?? 0, 0, ',', '.') }} đ</p>
                                        </div>
                                        <form action="{{ route('vouchers.remove') }}" method="POST">
                                            @csrf
                                ...
                                @else
                                    <button type="button" onclick="openVoucherModal('shipping')" class="w-full py-2 bg-primary/10 text-primary text-[13px] font-bold rounded-lg hover:bg-primary/20 transition-colors flex items-center justify-center gap-1.5">
                                        <span class="material-symbols-outlined text-[16px]">add_circle</span> Chọn hoặc nhập mã freeship
                                    </button>
                                @endif
                            </div>
                        </div>
...
<div id="voucherModal" class="fixed inset-0 z-[100] hidden bg-black/50 opacity-0 transition-opacity duration-300">
    <div id="voucherModalContent" class="fixed bottom-0 left-0 right-0 md:left-auto md:w-[450px] md:top-0 md:h-full bg-surface rounded-t-2xl md:rounded-l-2xl md:rounded-tr-none transform translate-y-full md:translate-y-0 md:translate-x-full transition-transform duration-300 flex flex-col max-h-[85vh] md:max-h-full">
        <div class="flex items-center justify-between p-4 border-b border-outline-variant/30">
            <h3 class="font-bold text-[18px]" id="voucherModalTitle">Chọn Mã Giảm Giá</h3>
        ...
        <div class="p-4 border-b border-outline-variant/30">
            <form action="{{ route('vouchers.apply') }}" method="POST" class="flex gap-2" id="manualVoucherForm">
                ...
                <input type="text" name="voucher_code" class="flex-1 p-3 rounded-xl bg-surface-container-low border border-outline-variant focus:border-primary focus:ring-0 text-[14px] transition-colors uppercase" placeholder="Nhập mã..." required>
                <button type="submit" class="px-4 btn-primary rounded-xl font-bold">
                    Áp dụng
                </button>
            </form>
        </div>
            ...
            @if(isset($availableVouchers) && count($availableVouchers) > 0)
                <form action="{{ route('vouchers.apply') }}" method="POST" id="voucherListForm" class="flex flex-col h-full m-0">
                            ...
                            <label class="voucher-item cursor-pointer block border border-outline-variant/30 rounded-xl p-4 bg-surface hover:bg-surface-container-lowest transition-colors relative" data-target="{{ $v->target ?? 'product' }}">
                                <div class="flex justify-between items-start">
                                    <div class="flex-1 pr-3">
                                        <p class="font-bold text-[15px] text-primary mb-1">{{ $v->code }}</p>
                                        <p class="text-[13px] font-semibold text-on-surface mb-1">{{ $v->name }}</p>
                                        <p class="text-[12px] text-on-surface-variant mb-2">{{ $v->description }}</p>
                                        <p class="text-[12px] text-on-surface-variant font-medium">Giảm {{ $v->discount_type == 'percent' ? $v->discount_value.'%' : number_format($v->discount_value, 0, ',', '.').'đ' }}
                    ...
                    <div class="p-4 border-t border-outline-variant/30 bg-surface sticky bottom-0" id="applyListVoucherContainer">
                        <button type="submit" class="btn-primary w-full py-3 rounded-xl font-bold text-[15px]" id="btnApplyListVoucher" disabled>
                            Áp dụng mã đã chọn
                        </button>
                    </div>
                </form>
                <div id="noVoucherMsg" class="hidden text-center py-8 text-on-surface-variant">
                    ...
                    <p class="text-[14px]">Không có mã giảm giá phù hợp</p>
                </div>
            @else
                <div class="text-center py-8 text-on-surface-variant">
                    <span class="material-symbols-outlined text-[48px] mb-2 opacity-50">local_activity</span>
                    <p class="text-[14px]">Bạn chưa lưu mã giảm giá nào.</p>
                </div>
            @endif
        </div>
    </div>
...
<script>
    ...
    function openVoucherModal(target) {
        voucherModalTitle.innerText = target === 'product' ? 'Chọn Mã Giảm Giá Sản Phẩm' : 'Chọn Mã Miễn Phí Vận Chuyển';
        ...
        if (voucherListContainer) {
            const items = voucherListContainer.querySelectorAll('.voucher-item');
            items.forEach(item => {
                const radio = item.querySelector('input[type="radio"]');
                if (radio) radio.checked = false;
                
                if (item.dataset.target === target || (target === 'product' && !item.dataset.target)) {
        ...
        if (noVoucherMsg) {
            noVoucherMsg.classList.toggle('hidden', hasVisibleVoucher);
        }
