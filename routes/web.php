...
Route::get('/', function (\Illuminate\Http\Request $request) {
    ...
    // Lấy danh sách vouchers còn hạn và được đánh dấu Hot
    $vouchers = \App\Models\Voucher::with('products')->where('status', 1)
        ->where('is_hot', true)
        ->where(function($q) {
    ...
    if (session()->has('user_id')) {
        $savedVoucherIds = \Illuminate\Support\Facades\DB::table('user_vouchers')
            ->where('user_id', session('user_id'))
            ->pluck('voucher_id')
            ->toArray();
    }

    return view('customer.home', compact('products', 'categories', 'isFiltered', 'vouchers', 'savedVoucherIds'));
});

Route::get('/customer/vouchers', function (\Illuminate\Http\Request $request) {
    // Lấy danh sách tất cả vouchers còn hạn (cả hot và không hot, hot lên đầu)
    $vouchers = \App\Models\Voucher::with('products')->where('status', 1)
        ->where(function($q) {
            $q->whereNull('start_date')->orWhere('start_date', '<=', now());
        })
    ...
    return view('customer.vouchers', compact('vouchers', 'savedVoucherIds'));
});

Route::post('/vouchers/save', function (\Illuminate\Http\Request $request) {
    if (!session()->has('user_id')) {
        return response()->json(['success' => false, 'message' => 'Vui lòng đăng nhập để lưu mã.']);
    }
...
Route::get('/admin/reports', function (\Illuminate\Http\Request $request) { 
    ...
    if ($period === 'custom') {
        ...
        $query->where('orders.created_at', '>=', now()->startOfYear());
    }

    $limit = $request->query('limit', 5);

    $topProductsQuery = $query->groupBy('products.id', 'products.name', 'products.image')
    ...
    return view('admin.reports', compact('topProducts', 'period')); 
});
