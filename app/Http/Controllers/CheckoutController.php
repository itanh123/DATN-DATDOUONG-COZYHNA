...
class CheckoutController extends Controller
{
    ...
    public function applyVoucher(Request $request)
    {
        ...
        // Kiểm tra xem user đã lưu mã này chưa và đã dùng chưa
        $savedVoucher = DB::table('user_vouchers')
            ->where('user_id', $userId)
            ->where('voucher_id', $voucher->id)
            ->first();

        if (!$savedVoucher) {
            return back()->with('voucher_error', 'Bạn chưa lưu mã giảm giá này. Hãy quay lại trang chủ để lưu mã!');
        }

        if ($savedVoucher->is_used) {
            return back()->with('voucher_error', 'Bạn đã sử dụng mã giảm giá này rồi.');
        }
        ...
        if ($customerProfile) {
            ...
            if ($cart) {
                $cartItems = DB::table('cart_items')
                    ->join('product_sizes', 'cart_items.product_size_id', '=', 'product_sizes.id')
                    ->where('cart_items.cart_id', $cart->id)
                    ->select('cart_items.quantity', 'product_sizes.selling_price as price', 'product_sizes.product_id')
                    ->get();
                foreach ($cartItems as $item) {
                    $cartTotal += $item->price * $item->quantity;
                }
        ...
        if ($voucher->discount_target === 'product') {
            $validProductIds = DB::table('voucher_products')->where('voucher_id', $voucher->id)->pluck('product_id')->toArray();
            $hasValidProduct = false;
            
            if (isset($cartItems)) {
                $cartItemProductIds = $cartItems->pluck('product_id')->toArray();
                if (count(array_intersect($validProductIds, $cartItemProductIds)) > 0) {
                    $hasValidProduct = true;
                }
            }

            if (!$hasValidProduct) {
                return back()->with('voucher_error', 'Giỏ hàng không chứa sản phẩm nào được áp dụng mã giảm giá này.');
            }
        }
        ...
        session([
            'voucher' => [
                ...
                'discount_target' => $voucher->discount_target ?? 'order',
    ...
    public function placeOrder(Request $request)
    {
        ...
        if ($appliedVoucher && $cartTotal >= $appliedVoucher['minimum_order']) {
            ...
            $discountTarget = $appliedVoucher['discount_target'] ?? 'order';
            
            $baseAmountForDiscount = $cartTotal;
            
            if ($discountTarget === 'product') {
                $validProductIds = DB::table('voucher_products')->where('voucher_id', $voucherId)->pluck('product_id')->toArray();
                ...
                $baseAmountForDiscount = $shippingFee;
            }

            if ($appliedVoucher['discount_type'] === 'percent') {
                $discountAmount = $baseAmountForDiscount * ($appliedVoucher['discount_value'] / 100);
                if ($appliedVoucher['maximum_discount'] && $discountAmount > $appliedVoucher['maximum_discount']) {
                    $discountAmount = $appliedVoucher['maximum_discount'];
                }
            } else {
