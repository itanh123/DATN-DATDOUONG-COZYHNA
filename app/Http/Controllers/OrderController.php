    ...
    public function placeOrder(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $profile, $cartItems, $userId, $checkoutItemIds, &$createdOrderCode) {
            ...
            if ($appliedVoucher) {
                ...
                // Increment voucher usage and mark as used for customer
                $voucher = \App\Models\Voucher::find($voucherId);
                if ($voucher) {
                    $voucher->increment('used_count');
                    $voucher->increment('used');
                    
                    // Mark as used in pivot table
                    $user = \App\Models\User::find($userId);
                    if ($user) {
                        $user->savedVouchers()->updateExistingPivot($voucherId, ['is_used' => true]);
                    }
                }
            }
            ...
            if ($appliedShippingVoucher) {
                $shippingVoucherId = $appliedShippingVoucher['id'];
                $sVoucher = \App\Models\Voucher::find($shippingVoucherId);
                if ($sVoucher) {
                    if ($sVoucher->discount_type === 'percent') {
                        $shippingDiscountAmount = ($shippingFee * $sVoucher->discount_value) / 100;
                        if ($sVoucher->maximum_discount && $shippingDiscountAmount > $sVoucher->maximum_discount) {
                            $shippingDiscountAmount = $sVoucher->maximum_discount;
                        }
                    } else {
                        $shippingDiscountAmount = $sVoucher->discount_value;
                    }

                    if ($shippingDiscountAmount > $shippingFee) {
                        $shippingDiscountAmount = $shippingFee;
                    }

                    // Increment voucher usage and mark as used for customer
                    $sVoucher->increment('used_count');
                    $sVoucher->increment('used');
                    
                    $user = \App\Models\User::find($userId);
                    if ($user) {
                        $user->savedVouchers()->updateExistingPivot($shippingVoucherId, ['is_used' => true]);
                    }
                }
            }
            
            $total = $subtotal + $shippingFee + $tax - $discount - $shippingDiscountAmount;
